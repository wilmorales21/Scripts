...
# Preencher as areas positivas com a cor vermelha
plt.fill_between(x1,y1,0,where=y1>0,color='red',interpolate=True)

# Preencher as areas positivas com a cor azul
plt.fill_between(x1,y1,0,where=y1<0,color='blue',interpolate=True)


# Colocando um titulo na figura
plt.title('Evolução mensal do ENSO')

# Colocando um titulo no eixo x
plt.xlabel('Anos')

# Colocando um titulo no eixo y
plt.ylabel('Índice de ENSO')

# Comando para plotar uma linha horizontal no meio da figura
plt.axhline(0.5, color='grey', lw= 0.5, linestyle= '-')
