...
concatenar = pd.concat([df['grupoa'],df['grupob'],df['valores_ajustados']], axis=1)

# Renomear as colunas
concatenar.rename(columns={'grupoa': 'Variável A', 'grupob': 'Variável B', 'valores_ajustados': 'Previsão'}, inplace=True)
print(concatenar)

#Criar uma planilha em Excel
regressao_excel = 'regressao_estatistica.xlsx'
concatenar.to_excel(regressao_excel, index=False)
